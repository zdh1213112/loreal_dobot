"""
功能说明：D405 末端局部精定位 + wait-lock + 传送带边缘拨散增强版。

主要流程：
1. 监听 ROS 2 话题 `/trigger_d405_vision`，收到 True 后启动一次 D405 精定位流程。
2. 监听 `/coarse_target_obj_for_d405`，触发后短暂等待最新粗定位锁定目标，
   用该目标辅助 D405 ROI/目标选择，避免使用过期粗定位信息。
3. 使用 RealSense D405 双红外 + FFS 生成深度点云，YOLO-OBB 定位目标，SAM2 分割目标区域。
4. 对目标点云做多平面 RANSAC 姿态估计，输出稳定的精抓位姿和夹爪宽度。
5. 根据目标 OBB、邻近点云和传送带边缘场景，计算目标两侧净空，判断是否需要拨散。
6. 选择更安全的拨散方向，并给出拨散起点、目标 X 向尺寸、拨散必要性等辅助信息。
7. 发布精定位抓取结果到 `/target_pose_cam_fine`，frame 为 `camera_d405_link`。
8. 发布夹爪开口宽度到 `/gripper_target_width`。
9. 发布拨散辅助话题：
   - `/target_singulation_hint`：拨散方向向量。
   - `/target_x_extent_cam`：目标在 D405 相机系下的 X 向尺寸。
   - `/target_singulation_start_cam`：建议拨散起点。
   - `/target_singulation_needed`：是否建议执行拨散。

典型配合：用于传送带边缘或堆叠/贴靠物体场景，给 Nova5 控制端提供预拨散后再抓取的信息。
"""
...
singulation_dir_pub = ros_node.create_publisher(Vector3Stamped, '/target_singulation_hint', 10)
target_x_extent_pub = ros_node.create_publisher(Float32, '/target_x_extent_cam', 10)
singulation_start_pub = ros_node.create_publisher(PoseStamped, '/target_singulation_start_cam', 10)
singulation_needed_pub = ros_node.create_publisher(Bool, '/target_singulation_needed', 10)
ros_node.declare_parameter("singulation_clearance_width_ratio", 0.10)  # 净空需求按夹取宽度的比例换算；越小越宽松
ros_node.declare_parameter("singulation_clearance_extra_margin_m", 0.00)  # 在比例净空之外额外增加的安全余量；越小越宽松
ros_node.declare_parameter("singulation_clearance_long_band_margin_m", 0.012)  # 沿目标长边分析邻近点云时额外放宽的范围
ros_node.declare_parameter("singulation_clearance_z_band_margin_m", 0.010)  # 沿目标高度分析邻近点云时额外放宽的范围
ros_node.declare_parameter("singulation_clearance_grip_zone_ratio", 0.20)  # 只检查长边中部真实夹取区域；越小越聚焦抓取带
ros_node.declare_parameter("singulation_clearance_percentile", 35.0)  # 用分位数代替最小值评估净空；越大越不容易被孤立近点触发
...
yolo_model = YOLO("/home/zdh/yolo_one/yolo_easy_deploy/outputs/train/obb_demo-13/weights/best.pt")
...
def choose_singulation_direction(obb_center, obb_R, filtered_points, obb_extent):
    long_axis = obb_R[:, 0]
    short_axis = obb_R[:, 1]
    local = (filtered_points - obb_center) @ obb_R

    long_half = max(1e-6, 0.5 * float(obb_extent[0]))
    short_half = max(1e-6, 0.5 * float(obb_extent[1]))
    edge_band = min(0.010, short_half * 0.45)

    pos_edge = filtered_points[(local[:, 1] >= short_half - edge_band)]
    neg_edge = filtered_points[(local[:, 1] <= -short_half + edge_band)]
    pos_density = len(pos_edge)
    neg_density = len(neg_edge)

    if pos_density + 2 < neg_density:
        push_dir = short_axis
    elif neg_density + 2 < pos_density:
        push_dir = -short_axis
    else:
        push_dir = None
    ...
    return push_dir, float(pos_density), float(neg_density), long_axis, short_axis


def edge_centers_from_obb(obb_corners_xy):
    corners = np.asarray(obb_corners_xy, dtype=np.float64).reshape(4, 2)
    edge_centers = []
    for i in range(4):
        p0 = corners[i]
        p1 = corners[(i + 1) % 4]
        edge_centers.append(0.5 * (p0 + p1))
    return np.asarray(edge_centers, dtype=np.float64)


def compute_side_clearance_m(scene_points, obb_center, obb_R, obb_extent):
    if scene_points is None or len(scene_points) == 0:
        return float("inf"), float("inf")

    local_scene = (scene_points - obb_center) @ obb_R
    half_long = 0.5 * float(obb_extent[0])
    half_short = 0.5 * float(obb_extent[1])
    half_height = 0.5 * float(obb_extent[2])

    long_margin = float(ros_node.get_parameter("singulation_clearance_long_band_margin_m").value)
    z_margin = float(ros_node.get_parameter("singulation_clearance_z_band_margin_m").value)
    grip_zone_ratio = float(ros_node.get_parameter("singulation_clearance_grip_zone_ratio").value)
    clearance_percentile = float(ros_node.get_parameter("singulation_clearance_percentile").value)
    grip_half_span = max(0.010, half_long * max(0.05, min(1.0, grip_zone_ratio)))

    band_mask = (
        (np.abs(local_scene[:, 0]) <= (grip_half_span + long_margin))
        & (np.abs(local_scene[:, 2]) <= (half_height + z_margin))
    )
    local_band = local_scene[band_mask]
    if len(local_band) == 0:
        return float("inf"), float("inf")

    pos_side = local_band[local_band[:, 1] > half_short]
    neg_side = local_band[local_band[:, 1] < -half_short]

    pos_clearance = float("inf")
    neg_clearance = float("inf")
    if len(pos_side) > 0:
        pos_clearance = float(np.percentile(pos_side[:, 1] - half_short, clearance_percentile))
    if len(neg_side) > 0:
        neg_clearance = float(np.percentile((-half_short) - neg_side[:, 1], clearance_percentile))
    return pos_clearance, neg_clearance
...
try:
    while True:
        ...
        background_points_for_clearance = None
        ...
        if current_mask is not None and np.any(current_mask):
            ...
            if np.any(highlight):
                ...
                background_points_for_clearance = points_3d[~highlight]
                ...
                if len(obj_pts) >= 10:
                    ...
                    if len(filtered) >= 10:
                        ...
                        if contours:
                            ...
                            target_x_extent_m = float(filtered[:, 0].max() - filtered[:, 0].min())
                            final_grip_position = target_grip_width_m + 0.020
                            singulation_dir, edge_pos_density, edge_neg_density, long_axis, short_axis = choose_singulation_direction(
                                obb_smooth_center,
                                obb_smooth_R,
                                filtered,
                                obb_smooth_extent,
                            )
                            positive_x_edge_center_uv = None
                            positive_x_edge_point = None
                            if last_yolo_obbs is not None and 0 <= last_best_idx < len(last_yolo_obbs) and len(uv_filtered) > 0:
                                edge_centers_uv = edge_centers_from_obb(last_yolo_obbs[last_best_idx])
                                edge_center_points_3d = []
                                for edge_center_uv in edge_centers_uv:
                                    edge_center_dists = np.linalg.norm(uv_filtered - edge_center_uv, axis=1)
                                    nearest_points = filtered[np.argsort(edge_center_dists)[:5]]
                                    edge_center_points_3d.append(np.mean(nearest_points, axis=0))
                                edge_center_points_3d = np.asarray(edge_center_points_3d, dtype=np.float64)
                                positive_idx = int(np.argmax(edge_center_points_3d[:, 0]))
                                positive_x_edge_center_uv = edge_centers_uv[positive_idx]
                                positive_x_edge_point = edge_center_points_3d[positive_idx]

                            if positive_x_edge_point is None:
                                positive_x_idx = int(np.argmax(filtered[:, 0]))
                                positive_x_edge_point = filtered[positive_x_idx]
                                positive_x_edge_center_uv = None
                            ...
                            pos_clearance_m, neg_clearance_m = compute_side_clearance_m(
                                background_points_for_clearance,
                                obb_smooth_center,
                                obb_smooth_R,
                                obb_smooth_extent,
                            )
                            clearance_width_ratio = float(
                                ros_node.get_parameter("singulation_clearance_width_ratio").value
                            )
                            clearance_extra_margin_m = float(
                                ros_node.get_parameter("singulation_clearance_extra_margin_m").value
                            )
                            grip_zone_ratio = float(
                                ros_node.get_parameter("singulation_clearance_grip_zone_ratio").value
                            )
                            clearance_percentile = float(
                                ros_node.get_parameter("singulation_clearance_percentile").value
                            )
                            required_clearance_m = (
                                target_grip_width_m * clearance_width_ratio + clearance_extra_margin_m
                            )
                            singulation_needed = (
                                pos_clearance_m < required_clearance_m
                                or neg_clearance_m < required_clearance_m
                            )
                            ...
                            if sam2_initialized:
                                ...
                                x_extent_msg = Float32()
                                x_extent_msg.data = float(target_x_extent_m)
                                target_x_extent_pub.publish(x_extent_msg)

                                start_msg = PoseStamped()
                                start_msg.header.stamp = ros_node.get_clock().now().to_msg()
                                start_msg.header.frame_id = "camera_d405_link"
                                start_msg.pose.position.x = float(positive_x_edge_point[0])
                                start_msg.pose.position.y = float(positive_x_edge_point[1])
                                start_msg.pose.position.z = float(positive_x_edge_point[2])
                                start_msg.pose.orientation.w = 1.0
                                singulation_start_pub.publish(start_msg)

                                singulation_needed_msg = Bool()
                                singulation_needed_msg.data = bool(singulation_needed)
                                singulation_needed_pub.publish(singulation_needed_msg)

                                hint_msg = Vector3Stamped()
                                hint_msg.header.stamp = ros_node.get_clock().now().to_msg()
                                hint_msg.header.frame_id = "camera_d405_link"
                                if singulation_dir is not None:
                                    hint_msg.vector.x = float(singulation_dir[0])
                                    hint_msg.vector.y = float(singulation_dir[1])
                                    hint_msg.vector.z = float(singulation_dir[2])
                                else:
                                    hint_msg.vector.x = 0.0
                                    hint_msg.vector.y = 0.0
                                    hint_msg.vector.z = 0.0
                                singulation_dir_pub.publish(hint_msg)

                                logging.info(
                                    "[D405 Singulation] "
                                    f"x_extent={target_x_extent_m:.3f}m short={obb_smooth_extent[1]:.3f}m "
                                    f"x_edge_center_uv=({0.0 if positive_x_edge_center_uv is None else positive_x_edge_center_uv[0]:.1f}, "
                                    f"{0.0 if positive_x_edge_center_uv is None else positive_x_edge_center_uv[1]:.1f}) "
                                    f"x_start=({positive_x_edge_point[0]:.3f}, {positive_x_edge_point[1]:.3f}, {positive_x_edge_point[2]:.3f}) "
                                    f"clearance_pos={pos_clearance_m:.3f}m clearance_neg={neg_clearance_m:.3f}m "
                                    f"required={required_clearance_m:.3f}m ratio={clearance_width_ratio:.2f} "
                                    f"extra_margin={clearance_extra_margin_m:.3f}m "
                                    f"grip_zone_ratio={grip_zone_ratio:.2f} percentile={clearance_percentile:.1f} "
                                    f"singulation_needed={singulation_needed} "
                                    f"dir=({0.0 if singulation_dir is None else singulation_dir[0]:.3f}, "
                                    f"{0.0 if singulation_dir is None else singulation_dir[1]:.3f}, "
                                    f"{0.0 if singulation_dir is None else singulation_dir[2]:.3f}) "
                                    f"edge_pos={edge_pos_density:.0f} edge_neg={edge_neg_density:.0f}"
                                )
                        else:
