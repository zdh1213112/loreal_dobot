...
# ===== 2. D405 不再加载 YOLO：使用粗定位 3D 先验动态生成 SAM2 ROI =====
...
try:
    while True:
        ...
        if global_trigger_flag:
            ...
            previous_size_time = locked_target_size_msg_time
            previous_corners_time = locked_target_corners_msg_time
            got_fresh_lock = wait_for_recent_locked_target(previous_lock_time)
            got_fresh_size = wait_for_recent_locked_size(previous_size_time, timeout_s=0.15)
            got_fresh_corners = wait_for_recent_locked_corners(previous_corners_time)
            if got_fresh_lock:
                logging.info("🔒 [D405 Lock] 已等到最新粗定位锁定目标，按粗定位先验生成 D405 ROI。")
            ...
            if got_fresh_corners:
                logging.info("🔷 [D405 Corners] 已等到最新粗定位角点，将优先使用 polygon ROI。")
            else:
                logging.warning("⚠️ [D405 Corners] 触发后未等到最新角点，才会回退 size ROI。")
            if got_fresh_size:
                logging.info("📏 [D405 Size] 已等到最新粗定位尺寸。")
            for _ in range(15):
            ...
            # 新触发只显示/使用最新一帧先验，先清掉旧 ROI 和旧 mask。
            last_prior_bbox = None
            last_prior_polygon = None
            current_mask = None
            prior_bbox = generate_prior_bbox_from_locked_corners()
            if prior_bbox is not None and last_prior_polygon is not None:
                last_prior_source = "polygon"
            if prior_bbox is None:
                prior_bbox = generate_prior_bbox_from_locked_target()
                last_prior_source = "size_fallback" if prior_bbox is not None else "none"
            if prior_bbox is not None:
                pending_bbox = prior_bbox
                last_prior_bbox = prior_bbox
                need_reset = True
                current_mask = None
                last_yolo_obbs = None
                last_best_idx = -1
                logging.info("✅ 使用粗定位 3D 先验 ROI 锁定目标，即将移交 SAM2 进行实时跟踪...")
                logging.info("="*50 + "\n")
            else:
                logging.warning("❌ 精定位失败：未能生成有效 D405 先验 ROI，请检查 /coarse_target_obj_for_d405。")
        ...
        if sam2_initialized:
            ...
            if current_mask is not None and last_prior_bbox is not None:
        ...
        if last_prior_bbox is not None:
