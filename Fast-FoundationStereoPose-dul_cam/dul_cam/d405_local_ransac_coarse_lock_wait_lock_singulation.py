"""
功能说明：D405 末端局部精定位 + wait-lock + 拨散方向提示版。
...
5. 根据目标 OBB 和周围点云，估计一个适合把目标从邻近物体中拨开的方向。
6. 发布精定位抓取结果到 `/target_pose_cam_fine`，frame 为 `camera_d405_link`。
7. 发布夹爪开口宽度到 `/gripper_target_width`。
8. 发布拨散方向提示到 `/target_singulation_hint`，供支持预拨散动作的 Nova5 控制端使用。

典型配合：用于目标相互贴近、需要先判断拨散方向再抓取的场景。
"""
                    ...
                    if len(filtered) >= 10:
                        ...
                        if contours:
                            ...
                            if sam2_initialized:
                                ...
                                logging.info(
                                    "[D405 Singulation] "
                                    f"long={obb_smooth_extent[0]:.3f}m short={obb_smooth_extent[1]:.3f}m "
