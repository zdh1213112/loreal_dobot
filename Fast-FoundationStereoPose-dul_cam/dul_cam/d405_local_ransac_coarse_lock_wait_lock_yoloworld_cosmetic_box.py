...
YOLO_WORLD_MODEL = os.environ.get("D405_YOLOWORLD_MODEL", "yolov8s-world.pt")
YOLO_WORLD_CLASSES = [os.environ.get("D405_YOLOWORLD_LABEL", "box")]
YOLO_WORLD_CONF = float(os.environ.get("D405_YOLOWORLD_CONF", "0.01"))
YOLO_WORLD_IOU = float(os.environ.get("D405_YOLOWORLD_IOU", "0.50"))
YOLO_WORLD_IMGSZ = int(os.environ.get("D405_YOLOWORLD_IMGSZ", "640"))
YOLO_WORLD_MAX_CENTER_DIST_PX = float(os.environ.get("D405_YOLOWORLD_MAX_CENTER_DIST_PX", "9999"))
YOLO_WORLD_MIN_AREA_PX = float(os.environ.get("D405_YOLOWORLD_MIN_AREA_PX", "150"))
YOLO_WORLD_MAX_AREA_RATIO = float(os.environ.get("D405_YOLOWORLD_MAX_AREA_RATIO", "0.28"))
YOLO_WORLD_MAX_WIDTH_RATIO = float(os.environ.get("D405_YOLOWORLD_MAX_WIDTH_RATIO", "0.70"))
YOLO_WORLD_MAX_HEIGHT_RATIO = float(os.environ.get("D405_YOLOWORLD_MAX_HEIGHT_RATIO", "0.70"))
YOLO_WORLD_EDGE_MARGIN_PX = int(os.environ.get("D405_YOLOWORLD_EDGE_MARGIN_PX", "3"))
YOLO_WORLD_BBOX_PADDING_PX = int(os.environ.get("D405_YOLOWORLD_BBOX_PADDING_PX", "12"))

logging.info(f"Loading YOLO-World model: {YOLO_WORLD_MODEL}")
try:
    yoloworld_model = YOLOWorld(YOLO_WORLD_MODEL)
except Exception as exc:
    raise RuntimeError(
        "YOLO-World 模型加载失败。请先下载 yolov8s-world.pt，或设置环境变量 "
        "D405_YOLOWORLD_MODEL=/path/to/yolov8s-world.pt 后再运行。"
    ) from exc
yoloworld_model.set_classes(YOLO_WORLD_CLASSES)
logging.info(f"YOLO-World classes set to: {YOLO_WORLD_CLASSES}")
...
def clamp_bbox_with_padding(x1, y1, x2, y2, padding=YOLO_WORLD_BBOX_PADDING_PX):
...
def select_yoloworld_box(color_bgr):
    results = yoloworld_model(color_bgr, conf=YOLO_WORLD_CONF, iou=YOLO_WORLD_IOU, imgsz=YOLO_WORLD_IMGSZ, verbose=False)
    if len(results) == 0 or results[0].boxes is None or len(results[0].boxes) == 0:
        logging.warning(f"[YOLO-World] 没有检测到任何 '{YOLO_WORLD_CLASSES[0]}'，conf={YOLO_WORLD_CONF} imgsz={YOLO_WORLD_IMGSZ}。")
        return None, None, None

    boxes_xyxy = results[0].boxes.xyxy.detach().cpu().numpy().astype(np.float64)
    ...
    confs = results[0].boxes.conf.detach().cpu().numpy().astype(np.float64)
    lock_uv = current_locked_target_uv()
    if lock_uv is None:
        lock_uv = (IMG_WIDTH * 0.5, IMG_HEIGHT * 0.5)
        logging.warning("[YOLO-World] 未拿到粗定位投影点，回退选择画面中心最近目标。")
    ...
    for idx, (box, conf) in enumerate(zip(boxes_xyxy, confs)):
        ...
        if area < YOLO_WORLD_MIN_AREA_PX or area > IMG_WIDTH * IMG_HEIGHT * YOLO_WORLD_MAX_AREA_RATIO:
            rejected.append(f"ID:{idx} area={area:.0f} conf={conf:.2f} dist={dist:.1f}")
            continue
        if width > IMG_WIDTH * YOLO_WORLD_MAX_WIDTH_RATIO or height > IMG_HEIGHT * YOLO_WORLD_MAX_HEIGHT_RATIO:
            rejected.append(f"ID:{idx} huge wh={width:.0f}x{height:.0f} conf={conf:.2f} dist={dist:.1f}")
            continue
        touches_edge = (
            x1 <= YOLO_WORLD_EDGE_MARGIN_PX
            or y1 <= YOLO_WORLD_EDGE_MARGIN_PX
            or x2 >= IMG_WIDTH - 1 - YOLO_WORLD_EDGE_MARGIN_PX
            or y2 >= IMG_HEIGHT - 1 - YOLO_WORLD_EDGE_MARGIN_PX
        )
        if touches_edge and area > IMG_WIDTH * IMG_HEIGHT * 0.08:
            rejected.append(f"ID:{idx} edge wh={width:.0f}x{height:.0f} conf={conf:.2f} dist={dist:.1f}")
            continue
        if dist > YOLO_WORLD_MAX_CENTER_DIST_PX:
            rejected.append(f"ID:{idx} far dist={dist:.1f} conf={conf:.2f} area={area:.0f}")
            continue
        size_penalty = area / max(IMG_WIDTH * IMG_HEIGHT, 1) * 80.0
        score = dist - float(conf) * 40.0 + size_penalty
        candidates.append((score, dist, float(conf), idx, box))

    if not candidates:
        logging.warning(
            f"[YOLO-World] 检测到 {len(boxes_xyxy)} 个 '{YOLO_WORLD_CLASSES[0]}'，但候选全被过滤；"
            f"lock_uv=({lock_uv[0]:.1f},{lock_uv[1]:.1f}) rejected={'; '.join(rejected[:6])}"
        )
        return None, box_polygons, None

    candidates.sort(key=lambda item: item[0])
    _, best_dist, best_conf, best_idx, best_box = candidates[0]
    bbox = clamp_bbox_with_padding(*best_box)
    if bbox is None:
        return None, box_polygons, None

    logging.info(
        f"🎯 [YOLO-World] 检测到 {len(boxes_xyxy)} 个 '{YOLO_WORLD_CLASSES[0]}'，"
        f"选择 ID:{best_idx} conf={best_conf:.2f} dist_to_lock={best_dist:.1f}px bbox={bbox}"
    )
...
try:
    while True:
        ...
        if global_trigger_flag:
            ...
            if got_fresh_lock:
                logging.info("🔒 [D405 Lock] 已等到最新粗定位锁定目标，将用其投影点选择最近的 YOLO-World cosmetic box。")
            elif locked_target_pose_msg is not None:
                logging.warning("⚠️ [D405 Lock] 触发后未等到更新锁定消息，继续沿用上一条锁定目标。")
            else:
                logging.warning("⚠️ [D405 Lock] 触发后未收到任何锁定目标，回退按画面中心选择 YOLO-World 目标。")
            for _ in range(15):
            ...
            last_prior_bbox = None
            last_prior_polygon = None
            last_prior_source = "yoloworld"
            current_mask = None

            selected_bbox, detected_boxes, selected_idx = select_yoloworld_box(color_bgr)
            ...
            if selected_bbox is not None:
                ...
                need_reset = True
                logging.info("✅ YOLO-World 锁定 cosmetic box，即将移交 SAM2 进行实时跟踪...")
                logging.info("="*50 + "\n")
            else:
                logging.warning("❌ 精定位失败：YOLO-World 未能选择到有效 cosmetic box。")
                last_best_idx = -1
        ...
        if sam2_initialized:
            ...
            if current_mask is not None and last_prior_bbox is not None and last_prior_source != "yoloworld":
                ...
                num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(current_mask, connectivity=8)
                if num_labels > 2:
                    keep_label = None
                    locked_uv = current_locked_target_uv()
                    if locked_uv is not None:
                        u_lock = int(round(np.clip(locked_uv[0], 0, IMG_WIDTH - 1)))
                        v_lock = int(round(np.clip(locked_uv[1], 0, IMG_HEIGHT - 1)))
                        label_at_lock = int(labels[v_lock, u_lock])
                        if label_at_lock > 0:
                            keep_label = label_at_lock
                    if keep_label is None:
                        areas = stats[1:, cv2.CC_STAT_AREA]
                        keep_label = int(np.argmax(areas) + 1)
                    current_mask = (labels == keep_label).astype(np.uint8)
