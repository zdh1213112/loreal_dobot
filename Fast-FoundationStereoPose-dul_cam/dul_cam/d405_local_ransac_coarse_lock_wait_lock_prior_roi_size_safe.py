...
PRIOR_ROI_SCALE = 1.15
PRIOR_ROI_MIN_HALF_W_PX = 45
PRIOR_ROI_MIN_HALF_H_PX = 80
PRIOR_ROI_MAX_HALF_W_PX = 110
PRIOR_ROI_MAX_HALF_H_PX = 190
    ...
    while True:
        ...
        if sam2_initialized:
            ...
            if current_mask is not None and last_prior_bbox is not None:
                ...
                roi_mask = np.zeros_like(current_mask, dtype=bool)
                roi_mask[y1:y2 + 1, x1:x2 + 1] = True
                current_mask = np.logical_and(current_mask > 0, roi_mask).astype(np.uint8)
