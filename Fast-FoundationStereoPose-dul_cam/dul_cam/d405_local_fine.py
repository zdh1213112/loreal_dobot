"""
RealSense D405 (末端精定位) + SAM2 + FFS + 触发式启动 + 完整姿态估计(SVD)
保留了全套 CV2 和 Open3D 实时可视化功能。
"""
...
yolo_model = YOLO("/home/zdh/ultralytics/runs/obb/train9/weights/best.pt")
...
config = rs.config()
config.enable_device('409122274792')  # 强制绑定 D405 序列号
...
try:
    while True:
        ...
        # =========================================================
        # 核心逻辑：拦截 ROS 触发信号并执行 YOLO 初始化
        # =========================================================
        if global_trigger_flag:
            logging.info(">>>> 正在清空历史运动残影，准备拍照... <<<<")
            # 丢弃运动时积压的 15 帧缓冲，确保抓取到绝对静止的高清图
            for _ in range(15):
            ...
            if len(results) > 0 and results[0].obb is not None and len(results[0].obb) > 0:
                # D405 俯视近景，通常画面中心即为目标，取置信度最高的目标
                best_idx = np.argmax(results[0].obb.conf.cpu().numpy())
                corners = results[0].obb.xyxyxyxy[best_idx].cpu().numpy()
        ...
        if need_reset:
            try:
                ...
                pass  # 兜底：如果 SAM2 还没吃过第一张图，底层字典为空，直接跳过报错
        ...
        if current_mask is not None and np.any(current_mask):
            ...
            if np.any(highlight):
                ...
                if len(obj_pts) >= 10:
                    ...
                    keep_mask_dist = dists <= np.percentile(dists, 90)
