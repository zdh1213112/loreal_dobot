...
PRIOR_ROI_SCALE = 1.35
PRIOR_ROI_MIN_HALF_W_PX = 70
PRIOR_ROI_MIN_HALF_H_PX = 55
PRIOR_ROI_MAX_HALF_W_PX = 240
PRIOR_ROI_MAX_HALF_H_PX = 180
PRIOR_DEPTH_GATE_M = 0.07
...
def generate_prior_bbox_from_locked_target():
    ...
    half = np.array([
        PRIOR_OBJECT_LENGTH_M * 0.5,
        PRIOR_OBJECT_WIDTH_M * 0.5,
        PRIOR_OBJECT_HEIGHT_M * 0.5,
    ], dtype=np.float64) * PRIOR_ROI_SCALE
    ...
    corners_cam = (transform[:3, :3] @ corners_local.T).T + transform[:3, 3]
    pixels = _project_points_to_pixels(corners_cam)
    ...
    if pixels is None:
        half_w = PRIOR_ROI_MIN_HALF_W_PX
        half_h = PRIOR_ROI_MIN_HALF_H_PX
    else:
        min_u, min_v = np.min(pixels, axis=0)
        max_u, max_v = np.max(pixels, axis=0)
        half_w = max(PRIOR_ROI_MIN_HALF_W_PX, 0.5 * (max_u - min_u))
        half_h = max(PRIOR_ROI_MIN_HALF_H_PX, 0.5 * (max_v - min_v))
    ...
    if x2 <= x1 or y2 <= y1:
        return None
    logging.info(
        f"💡 [D405 Prior ROI] center=({center_u:.1f},{center_v:.1f}) bbox=({x1},{y1},{x2},{y2}) "
        f"half=({half_w:.1f},{half_h:.1f}) z={transform[2, 3]:.3f}"
    )
    return (x1, y1, x2, y2)
