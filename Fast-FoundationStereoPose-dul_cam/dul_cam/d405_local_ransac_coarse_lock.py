"""
功能说明：D405 末端局部精定位基础版（带粗定位目标锁定）。

主要流程：
1. 监听 ROS 2 话题 `/trigger_d405_vision`，收到 True 后启动一次 D405 精定位流程。
2. 监听 `/coarse_target_obj_for_d405`，接收 Nova5 控制端发布的粗定位锁定目标，
   用于把 D405 检测范围约束到粗定位目标附近，避免误选其它物体。
3. 使用 RealSense D405 的双红外图像，经 FFS 模型计算稠密视差/深度点云。
4. 使用 YOLO-OBB 检测目标框，并结合 SAM2 分割目标区域。
5. 对目标点云做多平面 RANSAC，选择稳定主平面/最宽面来估计抓取姿态，
   减少物体随机摆放时姿态 Z 轴跳变。
6. 发布精定位抓取结果到 `/target_pose_cam_fine`，frame 为 `camera_d405_link`。
7. 发布估计夹爪开口宽度到 `/gripper_target_width`，供 Nova5/DH 夹爪控制端使用。

典型配合：D435 粗定位节点 -> Nova5 粗定位移动 -> 本节点 D405 精定位 -> Nova5 精抓。
"""
    ...
    while True:
        ...
        if global_trigger_flag:
            ...
            if len(results) > 0 and results[0].obb is not None and len(results[0].obb) > 0:
                ...
                last_locked_target_uv = project_locked_target_to_pixel()
        ...
        if last_locked_target_uv is not None:
            u_lock = int(round(last_locked_target_uv[0]))
            v_lock = int(round(last_locked_target_uv[1]))
