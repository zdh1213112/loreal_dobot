...
# ===== 2. D405 SAM2 point prompt from D435 coarse center =====
# 正点半径：控制正样本点围绕 D435 粗定位中心展开的范围，越大越容易吃到整盒，但也更容易沾到邻盒。
SAM2_POSITIVE_RADIUS_PX = int(os.environ.get("D405_SAM2_POSITIVE_RADIUS_PX", "18"))
# 负点半径：抑制目标周边相邻化妆品盒，太小容易切掉目标，太大抑制不住邻居。
SAM2_NEGATIVE_RADIUS_PX = int(os.environ.get("D405_SAM2_NEGATIVE_RADIUS_PX", "130"))
# 斜向负点半径：进一步压制角落方向的邻近盒子或背景。
SAM2_NEGATIVE_DIAGONAL_RADIUS_PX = int(os.environ.get("D405_SAM2_NEGATIVE_DIAGONAL_RADIUS_PX", "165"))
# 点提示 ROI 半宽：只影响调试/显示区域，不直接改变 SAM2 点本身。
SAM2_POINT_ROI_HALF_W_PX = int(os.environ.get("D405_SAM2_POINT_ROI_HALF_W_PX", "170"))
# 点提示 ROI 半高：只影响调试/显示区域，不直接改变 SAM2 点本身。
SAM2_POINT_ROI_HALF_H_PX = int(os.environ.get("D405_SAM2_POINT_ROI_HALF_H_PX", "170"))
# 从 D435 尺寸估算的粗框放大系数：越大越容易覆盖完整盒体，但也可能带进邻盒。
SAM2_BOX_SCALE = float(os.environ.get("D405_SAM2_BOX_SCALE", "1.65"))
# 粗框最小半宽：防止近距离目标框太窄。
SAM2_BOX_MIN_HALF_W_PX = int(os.environ.get("D405_SAM2_BOX_MIN_HALF_W_PX", "55"))
# 粗框最小半高：防止近距离目标框太矮。
SAM2_BOX_MIN_HALF_H_PX = int(os.environ.get("D405_SAM2_BOX_MIN_HALF_H_PX", "75"))
# 粗框最大半宽：防止框过度扩张，把邻近盒子一起包含进来。
SAM2_BOX_MAX_HALF_W_PX = int(os.environ.get("D405_SAM2_BOX_MAX_HALF_W_PX", "170"))
# 粗框最大半高：防止框过度扩张，把邻近盒子一起包含进来。
SAM2_BOX_MAX_HALF_H_PX = int(os.environ.get("D405_SAM2_BOX_MAX_HALF_H_PX", "210"))
# 绿色传送带判定阈值：G 通道明显高于 R/B 时，认为是背景传送带。
GREEN_BELT_G_MIN = int(os.environ.get("D405_GREEN_BELT_G_MIN", "80"))
GREEN_BELT_RG_DIFF = int(os.environ.get("D405_GREEN_BELT_RG_DIFF", "18"))
GREEN_BELT_BG_DIFF = int(os.environ.get("D405_GREEN_BELT_BG_DIFF", "12"))
# 如果粗定位点正好落在绿色带上，最多在周围搜索多少像素，把点吸附到最近的非绿色区域。
GREEN_BELT_SNAP_SEARCH_RADIUS_PX = int(os.environ.get("D405_GREEN_BELT_SNAP_SEARCH_RADIUS_PX", "28"))
...
def keep_component_near_locked_target(mask, color_bgr=None):
    ...
    keep_label = None
    locked_uv = current_locked_target_uv()
    component_scores = []
    for label_id in range(1, num_labels):
        area = stats[label_id, cv2.CC_STAT_AREA]
        if area < 20:
            continue
        x = stats[label_id, cv2.CC_STAT_LEFT] + stats[label_id, cv2.CC_STAT_WIDTH] * 0.5
        y = stats[label_id, cv2.CC_STAT_TOP] + stats[label_id, cv2.CC_STAT_HEIGHT] * 0.5
        score = 0.0
        if locked_uv is not None:
            score += np.hypot(x - locked_uv[0], y - locked_uv[1])
        score -= min(area, 8000) * 0.001
        if color_bgr is not None:
            xx = int(np.clip(round(x), 0, IMG_WIDTH - 1))
            yy = int(np.clip(round(y), 0, IMG_HEIGHT - 1))
            if is_green_belt_pixel(color_bgr, xx, yy):
                score += 60.0
        component_scores.append((score, label_id))

    if component_scores:
        keep_label = min(component_scores, key=lambda item: item[0])[1]
    if keep_label is None:
        areas = stats[1:, cv2.CC_STAT_AREA]
        keep_label = int(np.argmax(areas) + 1)
    return (labels == keep_label).astype(np.uint8)

def is_green_belt_pixel(color_bgr, x, y):
    if x < 0 or y < 0 or x >= color_bgr.shape[1] or y >= color_bgr.shape[0]:
        return False
    b, g, r = color_bgr[y, x].astype(np.int32)
    return g > GREEN_BELT_G_MIN and g > r + GREEN_BELT_RG_DIFF and g > b + GREEN_BELT_BG_DIFF

def snap_point_off_green_belt(color_bgr, x, y, search_radius=GREEN_BELT_SNAP_SEARCH_RADIUS_PX):
    x = int(np.clip(x, 0, IMG_WIDTH - 1))
    y = int(np.clip(y, 0, IMG_HEIGHT - 1))
    if not is_green_belt_pixel(color_bgr, x, y):
        return x, y
    best = None
    for radius in range(2, search_radius + 1, 2):
        for dx in range(-radius, radius + 1, 2):
            for dy in (-radius, radius):
                xx = int(np.clip(x + dx, 0, IMG_WIDTH - 1))
                yy = int(np.clip(y + dy, 0, IMG_HEIGHT - 1))
                if not is_green_belt_pixel(color_bgr, xx, yy):
                    score = radius + abs(dx) * 0.1
                    if best is None or score < best[0]:
                        best = (score, xx, yy)
            for dy in range(-radius + 2, radius - 1, 2):
                for xx, yy in ((int(np.clip(x - radius, 0, IMG_WIDTH - 1)), int(np.clip(y + dy, 0, IMG_HEIGHT - 1))),
                               (int(np.clip(x + radius, 0, IMG_WIDTH - 1)), int(np.clip(y + dy, 0, IMG_HEIGHT - 1)))):
                    if not is_green_belt_pixel(color_bgr, xx, yy):
                        score = radius + abs(dy) * 0.1
                        if best is None or score < best[0]:
                            best = (score, xx, yy)
        if best is not None:
...
def make_sam2_box_from_locked_target(u, v):
    depth = current_locked_target_depth()
    if depth is None or depth <= 1e-6:
        depth = 0.28
    size = np.sort(current_locked_target_size())[::-1]
    long_m = float(size[0])
    short_m = float(size[1]) if len(size) > 1 else long_m * 0.5
    half_w = 0.5 * short_m * K_color[0, 0] / depth * SAM2_BOX_SCALE
    half_h = 0.5 * long_m * K_color[1, 1] / depth * SAM2_BOX_SCALE
    half_w = min(SAM2_BOX_MAX_HALF_W_PX, max(SAM2_BOX_MIN_HALF_W_PX, half_w))
    half_h = min(SAM2_BOX_MAX_HALF_H_PX, max(SAM2_BOX_MIN_HALF_H_PX, half_h))
    return (
        int(np.clip(u - half_w, 0, IMG_WIDTH - 1)),
        int(np.clip(v - half_h, 0, IMG_HEIGHT - 1)),
        int(np.clip(u + half_w, 0, IMG_WIDTH - 1)),
        int(np.clip(v + half_h, 0, IMG_HEIGHT - 1)),
    )

def make_sam2_points_from_locked_target(color_bgr):
    lock_uv = current_locked_target_uv()
    if lock_uv is None:
        logging.warning("[SAM2 Point] 未拿到 D435 粗定位投影点，无法自动点提示。")
        return None, None, None

    u, v = snap_point_off_green_belt(color_bgr, int(round(lock_uv[0])), int(round(lock_uv[1])))
    points = [[u, v]]
    labels = [1]

    # 在中心附近补几个正点，让 SAM2 更容易吃住盒子表面，而不是只抓住纹理小块。
    for du, dv in [
        (-SAM2_POSITIVE_RADIUS_PX, 0),
        (SAM2_POSITIVE_RADIUS_PX, 0),
        (0, -SAM2_POSITIVE_RADIUS_PX),
        (0, SAM2_POSITIVE_RADIUS_PX),
    ]:
        points.append([np.clip(u + du, 0, IMG_WIDTH - 1), np.clip(v + dv, 0, IMG_HEIGHT - 1)])
        labels.append(1)

    # 一堆化妆品盒场景：围绕目标外圈放负点，抑制相邻盒子/背景被一起分割。
    for du, dv in [
        (-SAM2_NEGATIVE_RADIUS_PX, 0),
        (SAM2_NEGATIVE_RADIUS_PX, 0),
        (0, -SAM2_NEGATIVE_RADIUS_PX),
        (0, SAM2_NEGATIVE_RADIUS_PX),
        (-SAM2_NEGATIVE_DIAGONAL_RADIUS_PX, -SAM2_NEGATIVE_DIAGONAL_RADIUS_PX),
        (SAM2_NEGATIVE_DIAGONAL_RADIUS_PX, -SAM2_NEGATIVE_DIAGONAL_RADIUS_PX),
        (SAM2_NEGATIVE_DIAGONAL_RADIUS_PX, SAM2_NEGATIVE_DIAGONAL_RADIUS_PX),
        (-SAM2_NEGATIVE_DIAGONAL_RADIUS_PX, SAM2_NEGATIVE_DIAGONAL_RADIUS_PX),
    ]:
    ...
    roi = make_sam2_box_from_locked_target(u, v)
    return np.asarray(points, dtype=np.float32), np.asarray(labels, dtype=np.int32), roi
...
try:
    while True:
        ...
        if global_trigger_flag:
            ...
            if got_fresh_lock:
                logging.info("🔒 [D405 Lock] 已等到最新粗定位锁定目标，将用其投影点生成 SAM2 正负点提示。")
            elif locked_target_pose_msg is not None:
                logging.warning("⚠️ [D405 Lock] 触发后未等到更新锁定消息，继续沿用上一条锁定目标。")
            else:
                logging.warning("⚠️ [D405 Lock] 触发后未收到任何锁定目标，无法自动点提示。")
            for _ in range(15):
            ...
            last_prior_bbox = None
            last_prior_polygon = None
            last_prior_source = "sam2_point"
            current_mask = None
            last_yolo_obbs = None
            last_best_idx = -1

            prompt_points, prompt_labels, prompt_roi = make_sam2_points_from_locked_target(color_bgr)
            if prompt_points is not None:
                pending_point = (prompt_points, prompt_labels, prompt_roi)
                last_prior_bbox = prompt_roi
                need_reset = True
                logging.info(
                    f"✅ SAM2 点提示已生成: pos={int(np.count_nonzero(prompt_labels == 1))} "
                    f"neg={int(np.count_nonzero(prompt_labels == 0))} roi={prompt_roi}，即将跟踪目标..."
                )
                logging.info("="*50 + "\n")
            else:
                logging.warning("❌ 精定位失败：未能生成 SAM2 点提示，请检查 /coarse_target_obj_for_d405。")
        ...
        if sam2_initialized:
            ...
            if current_mask is not None and last_prior_bbox is not None and last_prior_source not in ("yoloworld", "sam2_point"):
        ...
        if last_prior_bbox is not None and last_prior_source != "yoloworld":
            ...
            if np.any(highlight):
                ...
                if prior_depth is not None:
                    ...
                    if np.count_nonzero(depth_keep) >= 10:
                        ...
                        logging.warning(
                            f"[D405 Prior ROI] 深度门控后点数不足 {np.count_nonzero(depth_keep)}，暂不应用深度过滤。"
                        )
