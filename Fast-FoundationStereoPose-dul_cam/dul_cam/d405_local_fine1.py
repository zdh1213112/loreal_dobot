...
新增：中心点优先策略、打印检测数量与决策原因、所有 YOLO 检测框可视化。
"""
    ...
    while True:
        ...
        if global_trigger_flag:
            ...
            if len(results) > 0 and results[0].obb is not None and len(results[0].obb) > 0:
                ...
                logging.info(f"💡 [选择依据] 目标 ID:{best_idx} 距离画面中心点(320, 240)最近 (距离平方: {dists[best_idx]:.1f})，设为最佳目标！")
                
                # 缓存数据用于后续 CV2 实时渲染显示所有的框
