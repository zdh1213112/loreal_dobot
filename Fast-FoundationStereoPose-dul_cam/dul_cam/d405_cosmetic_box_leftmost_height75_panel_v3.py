...
* while the robot approaches the grasp hover, optionally decodes a barcode only
  inside the selected YOLO/SAM target region and reports it on the cycle topic;
* embeds the V3 D435 turntable-barcode preview and forwards its ROI gestures.
"""
...
from dobot_nova5_driver.top_surface_geometry_v3 import (
...
YOLO_MODEL_PATH = "/home/zdh/yolo_one/yolo_train_web_github/outputs/train/obb_demo-13/weights/best.pt"
VALID_ITERS = 6
MAX_DISP = 192
ZNEAR = 0.16
ZFAR = 5.0
...
FAULT_SNAPSHOT_OUTPUT_DIR = os.path.join(
    os.path.dirname(D405_OUTPUT_DIR), "d405_fault_snapshots_v3"
)
...
TOP_BARCODE_DEBUG_OUTPUT_DIR = os.path.join(
    os.path.dirname(D405_OUTPUT_DIR), "d405_top_barcode_debug_v3"
)
...
LOCAL_WINDOW_NAME = "D405 RGB + Point Cloud + D435 Turntable Barcode"
...
D435_PANEL_LEFT = (LOCAL_TOP_WIDTH - D435_PANEL_DISPLAY_WIDTH) // 2
D435_PANEL_TOP = IMG_HEIGHT + LOCAL_DIVIDER_WIDTH_PX
LOCAL_WINDOW_HEIGHT = D435_PANEL_TOP + D435_PANEL_DISPLAY_HEIGHT
MIN_ROI_SIZE_PX = 20
...
rclpy.init()
ros_node = rclpy.create_node("d405_cosmetic_box_height75_node_v3")
...
d435_event_pub = ros_node.create_publisher(String, D435_EVENT_TOPIC, 10)
...
def combined_window_mouse_callback(event, x, y, flags, param) -> None:
    """Route ROI gestures to the D405 or embedded D435 image panel."""

    global combined_mouse_owner

    in_d405 = 0 <= x < IMG_WIDTH and 0 <= y < IMG_HEIGHT
    in_d435 = (
        D435_PANEL_LEFT <= x < D435_PANEL_LEFT + D435_PANEL_DISPLAY_WIDTH
        and D435_PANEL_TOP <= y < D435_PANEL_TOP + D435_PANEL_DISPLAY_HEIGHT
    )

    if event == cv2.EVENT_RBUTTONDOWN and in_d435:
        publish_d435_mouse_event(event, x, y, flags)
        return
    if event == cv2.EVENT_LBUTTONDOWN:
        if in_d405:
            combined_mouse_owner = "d405"
        elif in_d435:
            combined_mouse_owner = "d435"
        else:
            combined_mouse_owner = None

    if combined_mouse_owner == "d405":
        mouse_callback(
            event,
            max(0, min(IMG_WIDTH - 1, int(x))),
            max(0, min(IMG_HEIGHT - 1, int(y))),
    ...
    elif combined_mouse_owner == "d435":
        publish_d435_mouse_event(event, x, y, flags)
...
def publish_d435_mouse_event(event: int, x: int, y: int, flags: int) -> None:
    """Map combined-window coordinates back to the native D435 frame."""

    with d435_preview_lock:
        if latest_d435_preview is None:
            source_width = D435_PANEL_SOURCE_WIDTH
            source_height = D435_PANEL_SOURCE_HEIGHT
        else:
            source_height, source_width = latest_d435_preview.shape[:2]
    local_x = max(
        0,
        min(D435_PANEL_DISPLAY_WIDTH - 1, int(x) - D435_PANEL_LEFT),
    )
    local_y = max(
        0,
        min(D435_PANEL_DISPLAY_HEIGHT - 1, int(y) - D435_PANEL_TOP),
    )
    source_x = min(
        source_width - 1,
        int(round(local_x * source_width / D435_PANEL_DISPLAY_WIDTH)),
    )
    source_y = min(
        source_height - 1,
        int(round(local_y * source_height / D435_PANEL_DISPLAY_HEIGHT)),
    )
    message = String()
...
def compose_local_window(rgb_image: np.ndarray, cloud_image: np.ndarray) -> np.ndarray:
    """Compose D405 RGB/cloud and the D435 scanner into one local window."""
    ...
    top_row = np.hstack((rgb_image, vertical_divider, cloud_scaled))
    bottom_row = np.full(
        (D435_PANEL_DISPLAY_HEIGHT, top_row.shape[1], 3),
        18,
        dtype=np.uint8,
    )
    with d435_preview_lock:
        d435_image = (
            None if latest_d435_preview is None else latest_d435_preview.copy()
        )
        received_at = latest_d435_preview_received_at
    if d435_image is None:
        d435_scaled = np.full(
            (D435_PANEL_DISPLAY_HEIGHT, D435_PANEL_DISPLAY_WIDTH, 3),
            18,
            dtype=np.uint8,
        )
        cv2.putText(
            d435_scaled,
            "WAITING FOR D435 PREVIEW...",
            (90, D435_PANEL_DISPLAY_HEIGHT // 2),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.75,
            (0, 220, 255),
            2,
        ...
        d435_scaled = cv2.resize(
            d435_image,
            (D435_PANEL_DISPLAY_WIDTH, D435_PANEL_DISPLAY_HEIGHT),
            interpolation=cv2.INTER_AREA,
        )
        if time.monotonic() - received_at > 1.0:
            cv2.putText(
                d435_scaled,
                "D435 PREVIEW STALE",
                (16, D435_PANEL_DISPLAY_HEIGHT - 18),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.65,
                (0, 0, 255),
                2,
    ...
    bottom_row[
        :,
        D435_PANEL_LEFT : D435_PANEL_LEFT + D435_PANEL_DISPLAY_WIDTH,
    ] = d435_scaled
    horizontal_divider = np.full(
        (LOCAL_DIVIDER_WIDTH_PX, top_row.shape[1], 3),
        48,
        dtype=np.uint8,
    )
    return np.vstack((top_row, horizontal_divider, bottom_row))
...
def d435_preview_callback(msg: CompressedImage) -> None:
    global latest_d435_preview, latest_d435_preview_received_at
    try:
    ...
    if image is None or image.size == 0:
        return
    with d435_preview_lock:
        latest_d435_preview = image
        latest_d435_preview_received_at = time.monotonic()
...
ros_node.create_subscription(
    CompressedImage, D435_PREVIEW_TOPIC, d435_preview_callback, 2
)
...
if ENABLE_LOCAL_WINDOWS:
    ...
    cv2.resizeWindow(LOCAL_WINDOW_NAME, LOCAL_TOP_WIDTH, LOCAL_WINDOW_HEIGHT)
