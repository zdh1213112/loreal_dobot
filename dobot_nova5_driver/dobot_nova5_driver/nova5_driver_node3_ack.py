...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("d405_trigger_delay_s", 0.3)
        self.declare_parameter("fine_refine_delay_s", 0.2)
        self.declare_parameter("fine_refine_cycles", 2)
        self.declare_parameter("fine_target_timeout_s", 2.0)
        self.declare_parameter("fine_post_trigger_settle_s", 0.6)
        self.declare_parameter("fine_position_stability_threshold_m", 0.02)
    ...
    def _wait_for_fine_target_settle(self, start_count: int, timeout_s: float, settle_s: float) -> list[TcpPose]:
        deadline = time.time() + timeout_s
        last_count = start_count
        last_update_time = time.time()
        collected: list[TcpPose] = []

        while time.time() < deadline:
            current_count = self.target_msg_count()
            if current_count > last_count:
                ...
                last_update_time = time.time()
            elif collected and (time.time() - last_update_time) >= settle_s:
                return collected
            time.sleep(0.02)
        ...
        if collected:
            return collected
        raise TimeoutError(
            f"Timed out waiting for fine target settle after trigger burst; start_count={start_count}"
        )
    ...
    def execute_coarse_then_fine_sequence(self) -> None:
        ...
        fine_post_trigger_settle_s = float(self.get_parameter("fine_post_trigger_settle_s").value)
        fine_refine_delay_s = float(self.get_parameter("fine_refine_delay_s").value)
        total_triggers = 1 + refine_cycles
        burst_start_count = self.target_msg_count()

        for cycle_index in range(total_triggers):
            trigger_msg = Bool()
            ...
            if cycle_index == 0:
                ...
                self.get_logger().info(
                    f"Published D405 trigger true for fine refine cycle {cycle_index}/{refine_cycles}."
                )
                time.sleep(fine_refine_delay_s)

        fine_samples = self._wait_for_fine_target_settle(
            burst_start_count,
            fine_target_timeout_s,
            fine_post_trigger_settle_s,
        )
        self.get_logger().info(
            f"Collected {len(fine_samples)} fine target updates after trigger burst before fine motion."
        )
