...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("post_grasp_rotate_step_deg", 90.0)
        self.declare_parameter("post_grasp_rotate_count", 3)
    ...
    def execute_post_grasp_place_sequence(self, transfer_pose: TcpPose, place_pose: TcpPose) -> None:
        ...
        self._move_linear_with_log(transfer_pose, "Moved to post-grasp transfer pose")

        for rotate_index in range(rotate_count):
            rotated_pose = TcpPose(
                x=transfer_pose.x,
                y=transfer_pose.y,
                z=transfer_pose.z,
                rx=transfer_pose.rx,
                ry=transfer_pose.ry,
                rz=normalize_angle_deg(transfer_pose.rz + rotate_step_deg * float(rotate_index + 1)),
            )
            self._move_linear_with_log(
                rotated_pose,
        ...
        self._move_linear_with_log(place_pose, "Moved to post-grasp place pose")
        if self._dh_gripper is not None:
            self._open_gripper_fully(wait=True)
