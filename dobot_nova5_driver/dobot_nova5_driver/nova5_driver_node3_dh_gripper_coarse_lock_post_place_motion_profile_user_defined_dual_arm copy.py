...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("secondary_robot_ip", "192.168.142.101")
        ...
        self.declare_parameter("secondary_startup_speed", 35)
        self.declare_parameter("secondary_joint_speed", 30)
        self.declare_parameter("secondary_joint_acc", 30)
        self.declare_parameter(
    ...
    def active_read_current_pose(self) -> TcpPose:
        return self._controller_for_robot(self._active_robot_key).read_pose()

    def active_current_tcp_pose(self) -> TcpPose:
        return self._controller_for_robot(self._active_robot_key).current_tcp_pose()
    ...
    def read_secondary_current_pose(self) -> TcpPose:
        if self._secondary_controller is None:
            raise RuntimeError("Secondary robot is not connected")
        return self._secondary_controller.read_pose()

    def execute_secondary_target(self, pose: TcpPose) -> None:
        if self._secondary_controller is None:
            raise RuntimeError("Secondary robot is not connected")
        with self._secondary_motion_lock:
            self._secondary_controller.move_joint_tcp(
                pose,
                speed=int(self.get_parameter("secondary_joint_speed").value),
                accel=int(self.get_parameter("secondary_joint_acc").value),
            )
        self.get_logger().info(
            f"Secondary robot moved to sync target: x={pose.x:.3f} y={pose.y:.3f} z={pose.z:.3f} "
    ...
    def execute_coarse_then_fine_sequence(
        ...
        secondary_pose: Optional[TcpPose] = None,
        ...
        if success:
            ...
            if secondary_pose is not None:
                ...
                self.execute_secondary_target(secondary_pose)
...
class Nova5ControlWindow(QMainWindow):
    def __init__(self, node: Nova5DriverNode):
        ...
        self.secondary_target_spinboxes: dict[str, QDoubleSpinBox] = {}
        ...
        self.setWindowTitle("Nova5 Dual-Arm Driver Control - User Defined Sync Target")
    ...
    def _build_secondary_target_group(self, parent_layout: QVBoxLayout) -> None:
        box = QGroupBox("副臂101协同目标位姿")
        layout = QGridLayout(box)

        info = QLabel(f"副臂 IP: {self.node.secondary_robot_ip()}，102 抓取成功后移动到这里。")
        info.setWordWrap(True)
        layout.addWidget(info, 0, 0, 1, 2)
        self._build_pose_spinbox_columns(layout, start_row=1, column_offset=0, store=self.secondary_target_spinboxes)

        self.write_secondary_target_button = QPushButton("副臂当前位姿写入目标")
    ...
    def _load_default_post_grasp_targets(self) -> None:
        ...
        self._set_pose_spinboxes(self.transfer_target_spinboxes, TcpPose(*transfer_values))
        self._set_pose_spinboxes(self.place_target_spinboxes, TcpPose(*place_values))
    ...
    def sync_secondary_feedback_to_target(self) -> None:
        try:
            pose = self.node.read_secondary_current_pose()
        except Exception as exc:
            self.set_message(f"Read secondary current pose failed: {exc}")
            return
        self._set_pose_spinboxes(self.secondary_target_spinboxes, pose)
        self.set_message("Secondary current pose written into sync target")
    ...
    def execute_secondary_target_only(self) -> None:
        if not self._secondary_target_configured:
            ...
            return
        secondary_pose = self._pose_from_spinboxes(self.secondary_target_spinboxes)
        self.run_action("ExecuteSecondaryTarget", self.node.execute_secondary_target, secondary_pose)
    ...
    def _build_pose_spinbox_columns(
        ...
        for row, axis in enumerate(POSE_AXES, start=start_row):
            ...
            elif store is self.secondary_target_spinboxes:
