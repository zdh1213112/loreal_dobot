...
def remap_camera_euler_deg(rx_deg: float, ry_deg: float, rz_deg: float) -> tuple[float, float, float]:
    # Empirical axis remap requested by the user:
    # +Ry -> camera -Rx, +Rx -> camera -Ry, +Rz -> camera -Rz
    return -ry_deg, -rx_deg, -rz_deg


def pose_from_xyzrxyz_to_transform(pose: TcpPose) -> np.ndarray:
    rotation = euler_deg_to_rotation_matrix(pose.rx, pose.ry, pose.rz)
    return make_transform(rotation, (pose.x, pose.y, pose.z))
...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("target_z_lift_m", 0.23) #夹爪差值补偿23cm
        self.declare_parameter("camera_frame_id", "camera_d405_link")
        self.declare_parameter(
            "handeye_flange_to_cam", #d405手在眼上标定
            [
    ...
    def _target_pose_callback(self, msg: PoseStamped) -> None:
        ...
        try:
            ...
            #     current_tcp = self.current_tcp_pose()
            #     base_flange = tcp_pose_to_transform(current_tcp)
            #     mapped_rx_deg, mapped_ry_deg, mapped_rz_deg = remap_camera_euler_deg(
            #         raw_target.rx, raw_target.ry, raw_target.rz
            #     )
            #     cam_target_pose = TcpPose(
            #         x=raw_target.x,
            #         y=raw_target.y,
            #         z=raw_target.z,
            #         rx=mapped_rx_deg,
            ...
            #         rz=mapped_rz_deg,
            #     )
            #     cam_target = pose_from_xyzrxyz_to_transform(cam_target_pose)
            #     base_target = base_flange @ self._handeye_flange_to_cam @ cam_target
            #     target = transform_to_tcp_pose(base_target)
            #     source = f"{self.get_parameter('motion_topic').value} [camera(remap)->base]"
            elif frame_id == self._camera_frame_id:
                # 1. 获取拍照瞬间的基座到法兰的变换
                current_tcp = self.current_tcp_pose()
                ...
                # 3. 计算物体在 Base 下的实际位姿 (Z轴朝上)
                T_base_to_target_obj = T_base_to_flange @ self._handeye_flange_to_cam @ T_cam_to_target
                
                # ==========================================
                # 核心修复：引入抓取姿态偏置 (Grasp Offset)
                # ==========================================
                # 物体的 Z 轴朝上，但我们需要机械臂 TCP 的 Z 轴朝下，所以绕 X 轴旋转 180 度 (翻转夹爪)
                # 物体的 Rz 算出来是 45度，但我们期望是 -58度，差了约 -103度（接近 -90度），所以绕 Z 轴旋转补偿偏航角
                # 你可能需要根据实际夹爪的方向，把 -90.0 改成 90.0 或 0.0 进行一次实机微调
                
                # 创建一个旋转矩阵: 绕 X轴旋转 180度, 绕 Z轴旋转 -90度
            ...
            # elif frame_id == self._camera_frame_id:
            #     # 1. 获取当前法兰盘 (flange/tool0) 在基座 (base) 下的 4x4 变换矩阵
            #     current_tcp = self.current_tcp_pose()
            #     T_base_to_flange = tcp_pose_to_transform(current_tcp)
                
            #     # 2. 将视觉传来的 PoseStamped (基于相机光学坐标系) 直接、安全地转为 4x4 矩阵
            #     # 这一步直接利用四元数和 XYZ，避免了任何欧拉角带来的奇异性和错乱
            #     T_cam_to_target = pose_stamped_to_transform(msg)
                
            #     # 3. 核心空间几何：通过齐次变换矩阵连乘，将目标映射到 Base 坐标系
            #     T_base_to_target = T_base_to_flange @ self._handeye_flange_to_cam @ T_cam_to_target
                
            #     # 4. 把计算好的 4x4 矩阵还原成 Nova5 机械臂接受的 [x, y, z, rx, ry, rz]
            #     target = transform_to_tcp_pose(T_base_to_target)
            #     source = f"{self.get_parameter('motion_topic').value} [camera_optical->base]"    
            else:
