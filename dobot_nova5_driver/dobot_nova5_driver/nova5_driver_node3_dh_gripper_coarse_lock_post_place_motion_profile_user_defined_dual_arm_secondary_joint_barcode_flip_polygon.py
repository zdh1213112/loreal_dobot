...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("barcode_wait_timeout_s", 2.0)
        ...
        self.declare_parameter(
            ...
            "handeye_base_to_d435",
            [
                0.9938108, 0.10750777, -0.02796736, 0.06212362452,
                0.10480691, -0.99087883, -0.08470334, 0.63295122513,
                -0.03681853, 0.08124792, -0.99601364, 1.3125790433,
                0.0, 0.0, 0.0, 1.0,
            ],
    ...
    def _check_and_flip_until_barcode_face_up(
        ...
        for attempt in range(max_steps + 1):
            ...
            if self._wait_for_stable_barcode_detection(wait_timeout_s):
                self.get_logger().info(
                    f"Barcode face confirmed stable on check {attempt + 1}/{max_steps + 1} "
                    f"with {self._barcode_stable_hit_count()} detections; starting face-up rotation."
                )
                ...
                if abs(face_up_after_detect_deg) > 1e-6:
                    ...
                    self.get_logger().info(
                        f"Barcode detected on check {attempt + 1}/{max_steps + 1}; applying user-frame "
                    ...
                    if not face_up_ok:
                        message = (
                            f"Barcode face-up TCP-tip local axis failed after stable detection on check "
                            f"{attempt + 1}/{max_steps + 1}."
                        )
