...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("default_secondary_target_joint", [-47.58, -26.70, -112.4522, 137.427, 68.152, -90.65]) #副臂101协同目标关节角度
        self.declare_parameter(
    ...
    def execute_secondary_target_joint(self, joints_deg: list[float]) -> None:
        if self._secondary_controller is None:
            raise RuntimeError("Secondary robot is not connected")
        if len(joints_deg) != 6:
            raise ValueError(f"secondary target joint must contain 6 values, got {len(joints_deg)}")
        ...
        self.get_logger().info(
            "Secondary robot moved to sync joint target: "
...
class Nova5ControlWindow(QMainWindow):
    def __init__(self, node: Nova5DriverNode):
        ...
        self.setWindowTitle("Nova5 Dual-Arm Driver Control - Secondary Joint Sync Target")
