"""V3 controller fork used by the turntable-enabled cosmetic-box cycle."""
...
from .TCP_IP_Python_V4.dobot_api import DobotApiDashboard, DobotApiFeedBack, MyType
...
DASHBOARD_REPLY_TIMEOUT_S = 5.0
# A transient feedback read error must not itself consume the entire 50 ms
# secondary-arm safety freshness budget.  A persistent error still leaves the
# feedback stale, so the existing interlock stops 101.
FEEDBACK_READ_ERROR_RETRY_S = 0.005
ROBOT_MODE_TEXT = {
    1: "INIT",
    2: "BRAKE_OPEN",
    3: "POWEROFF",
...
class BoundedDobotApiDashboard(DobotApiDashboard):
    """V3 Dashboard client with a bounded request/reply wait.

    The vendor client performs a blocking ``recv`` without a timeout.  If one
    reply is lost, the GUI worker that issued GetDO/GetPose/ClearError remains
    alive forever and every later action is refused as already running.  V3
    closes and recreates that socket after five seconds, then reports the
    failed command to the caller.  Feedback port 30004 is unaffected.
    """

    def __init__(self, ip: str, port: int) -> None:
        super().__init__(ip, port)
        self._v3_io_lock = threading.Lock()
        self._set_reply_timeout()

    def _set_reply_timeout(self) -> None:
        if self.socket_dobot not in (None, 0):
            self.socket_dobot.settimeout(DASHBOARD_REPLY_TIMEOUT_S)

    def _replace_socket(self) -> None:
        old_socket = self.socket_dobot
        if old_socket not in (None, 0):
            try:
                old_socket.close()
            except OSError:
                pass
        replacement = socket.create_connection(
            (self.ip, self.port),
            timeout=DASHBOARD_REPLY_TIMEOUT_S,
        )
        replacement.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 144000)
        replacement.settimeout(DASHBOARD_REPLY_TIMEOUT_S)
        self.socket_dobot = replacement

    def sendRecvMsg(self, string):  # noqa: N802 - vendor API name
        command = str(string)
        with self._v3_io_lock:
            try:
                self._set_reply_timeout()
                self.socket_dobot.sendall(command.encode("utf-8"))
                data = self.socket_dobot.recv(1024)
                if not data:
                    raise ConnectionError("Dashboard socket closed without a reply")
                ...
                detail = f"; reconnect failed: {reconnect_error}" if reconnect_error else ""
                raise TimeoutError(
                    f"Dashboard command {command!r} did not return within "
                    f"{DASHBOARD_REPLY_TIMEOUT_S:.1f}s{detail}"
                ) from exc


class FramedDobotApiFeedBack(DobotApiFeedBack):
    """Read complete 30004 frames without discarding TCP stream fragments.

    A TCP recv can contain part of a frame or several frames.  The vendor
    feedBackData implementation replaces short reads and discards long reads,
    so normal packet boundaries can appear as missing feedback.  Keep the
    remainder and publish the newest complete frame in each read instead.
    The length field is checked so a truncated stream cannot become a pose.
    """

    def __init__(self, ip: str, port: int) -> None:
        super().__init__(ip, port)
        self._feedback_buffer = bytearray()
        self.last_chunk_monotonic_s = 0.0

    def feedBackData(self):  # noqa: N802 - vendor API name
        frame_size = MyType.itemsize
        frame_header = frame_size.to_bytes(2, "little")
        latest_frame = None
        while True:
            if len(self._feedback_buffer) < frame_size:
                if latest_frame is not None:
                    break
                chunk = self.socket_dobot.recv(144000)
                if not chunk:
                    raise ConnectionError("30004 feedback socket closed")
                self.last_chunk_monotonic_s = time.monotonic()
                self._feedback_buffer.extend(chunk)
                continue

            if self._feedback_buffer[:2] != frame_header:
                next_header = self._feedback_buffer.find(frame_header, 1)
                if next_header < 0:
                    # Keep the final byte in case the two-byte header is
                    # split across consecutive TCP reads.
                    del self._feedback_buffer[:-1]
                else:
                    del self._feedback_buffer[:next_header]
                continue

            latest_frame = bytes(self._feedback_buffer[:frame_size])
            del self._feedback_buffer[:frame_size]
        return np.frombuffer(latest_frame, dtype=MyType)
...
class DobotNova5Controller:
    def __init__(
        ...
        self.dashboard: BoundedDobotApiDashboard | None = None
        self.feedback: FramedDobotApiFeedBack | None = None
        ...
        self._latest_feedback_read_error = ""
        self._feedback_read_active = False
        self._feedback_read_started_monotonic_s = 0.0
    ...
    def connect(self, go_to_start: bool = False, auto_enable: bool = False) -> None:
        self.dashboard = BoundedDobotApiDashboard(
            self.robot_ip,
            self.dashboard_port,
        )
        self.feedback = FramedDobotApiFeedBack(self.robot_ip, self.feedback_port)
    ...
    @property
    def feedback_last_read_error(self) -> str:
        """Report the most recent reader failure until a valid frame arrives."""

        with self._feedback_lock:
            return self._latest_feedback_read_error

    @property
    def feedback_reader_alive(self) -> bool:
        """Report whether the feedback receive thread can still make progress."""

        return bool(self._feedback_thread and self._feedback_thread.is_alive())

    def feedback_reader_diagnostics(self) -> str:
        """Snapshot receive progress for a 102 freshness-stop log."""

        now = time.monotonic()
        reader = self.feedback
        chunk_at = getattr(reader, "last_chunk_monotonic_s", 0.0)
        chunk_age_ms = (now - chunk_at) * 1000.0 if chunk_at > 0.0 else -1.0
        wait_ms = (
            (now - self._feedback_read_started_monotonic_s) * 1000.0
            if self._feedback_read_active
            else 0.0
        )
        buffered = len(getattr(reader, "_feedback_buffer", ()))
        thread_alive = bool(self._feedback_thread and self._feedback_thread.is_alive())
        return (
            f"reader_active={self._feedback_read_active}, "
            f"read_wait={wait_ms:.0f}ms, last_chunk_age={chunk_age_ms:.0f}ms, "
            f"buffered={buffered}B, thread_alive={thread_alive}"
        )
    ...
    def read_digital_output(self, index: int) -> int:
        """Read one controller-cabinet DO while serializing Dashboard access."""
        ...
        with self._command_lock:
            values = self._raise_if_error(
                self.dashboard.GetDO(index),
                f"GetDO({index})",
            )
        if len(values) != 1 or values[0] not in ("0", "1"):
            raise RuntimeError(
                f"GetDO({index}) returned an invalid state: {values!r}"
            )
        return int(values[0])
    ...
    def pulse_digital_output(self, index: int, pulse_ms: int = 300) -> None:
        ...
        with self._command_lock:
            try:
                ...
                self._raise_if_error(
                    self.dashboard.DOInstant(index, 0),
                    f"DOInstant({index},0) pulse finish",
                )
                output_high = False
            finally:
                # Returning the electrical output low does not change the
                # turntable again; only its rising edge toggles run/stop.
                if output_high:
                    try:
                        self.dashboard.DOInstant(index, 0)
                    except Exception:
    ...
    def forward_kinematics(
        ...
        """Return the User/Tool TCP pose of a joint target without moving."""
        ...
        with self._command_lock:
            response = self.dashboard.PositiveKin(
                *(float(value) for value in joints_deg),
                user=-1 if user_index is None else int(user_index),
                tool=-1 if tool_index is None else int(tool_index),
            )
        values = self._raise_if_error(response, "PositiveKin")
        if len(values) < 6:
            raise RuntimeError(f"PositiveKin did not return 6 pose values: {response.strip()}")
        return TcpPose(
            float(values[0]) / MM_PER_METER,
            float(values[1]) / MM_PER_METER,
            float(values[2]) / MM_PER_METER,
            float(values[3]),
            float(values[4]),
            float(values[5]),
        )
    ...
    def _feedback_loop(self) -> None:
        while not self._stop_feedback.is_set() and self.feedback is not None:
            try:
                self._feedback_read_started_monotonic_s = time.monotonic()
                self._feedback_read_active = True
                ...
                if not self._stop_feedback.is_set():
                    time.sleep(FEEDBACK_READ_ERROR_RETRY_S)
