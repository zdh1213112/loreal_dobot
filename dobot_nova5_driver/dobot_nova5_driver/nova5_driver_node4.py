...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("fine_tool_to_flange_z_m", 0.225) # 末端D405抓取时，夹爪尖端到法兰中心Z差值
        ...
        self.declare_parameter("coarse_target_x_offset_m", 0.08)  # 粗定位最终base执行值在x上减10cm
        ...
        self.declare_parameter("d405_trigger_delay_s", 0.5)
        self.declare_parameter("fine_refine_delay_s", 0.2)
        ...
        self.declare_parameter("fine_angle_stability_threshold_deg", 11.0)

        #  加上这行：精定位误差超限时的最大重试次数
    ...
    def execute_coarse_then_fine_sequence(self) -> None:
        ...
        # [阶段 1] 粗定位移动与到达检测
        # ==============================================================
        self.get_logger().info("========== [阶段 1] 开始粗定位移动 ==========")
        with self._motion_lock:
            self._controller.move_linear_tcp(
                coarse_target,
                speed=int(self.get_parameter("linear_speed").value),
        ...
        # 阻塞等待机械臂真正移动到粗定位悬停点 (误差 1cm 内认为到达)
        self.get_logger().info("--> 等待机械臂到达粗定位悬停点...")
        start_time = time.time()
        while time.time() - start_time < 20.0:
            try:
                curr = self.current_tcp_pose()
                dist = math.sqrt((curr.x - coarse_target.x)**2 + (curr.y - coarse_target.y)**2 + (curr.z - coarse_target.z)**2)
                if dist < 0.01: 
                    break
            except Exception:
                pass
            time.sleep(0.2)
        self.get_logger().info("--> 粗定位已就位！")

        # ==============================================================
        # [阶段 2] 精定位循环采样与稳定性重试机制 (Vision Retry Logic)
        # ==============================================================
        ...
        d405_trigger_delay_s = float(self.get_parameter("d405_trigger_delay_s").value)
        fine_refine_delay_s = float(self.get_parameter("fine_refine_delay_s").value)
        
        try:
            max_retries = int(self.get_parameter("fine_max_retries").value)
        except Exception:
            max_retries = 3  # 兜底默认重试3次

        success = False
        self.get_logger().info("========== [阶段 2] 启动精定位视觉采样 ==========")
        
        # 开始尝试循环（包含首次尝试 + max_retries 次重试）
        for attempt in range(max_retries + 1):
            if attempt > 0:
                self.get_logger().warning(f"--> [重试机制] 正在进行第 {attempt} 次重新采样...")
            ...
            time.sleep(d405_trigger_delay_s)
            # 新增：休眠结束后再记录 count，确保只抓取 YOLO 算完后的全新一帧
            previous_count = self.target_msg_count()
            try:
                ...
                continue # 直接跳过本次剩余步骤，触发下一次 Retry
            ...
            if not refine_success:
                continue # Timeout 发生，触发下一次 Retry

            # 3. 核心：捕获稳定性校验异常
            try:
                self._validate_fine_target_stability(fine_samples)
                
                # 如果代码走到这里，说明没有抛出 RuntimeError，位姿非常稳定！
                ...
                self.get_logger().info(f"--> ✅ 校验通过！共平均了 {len(fine_samples)} 帧有效位姿。")
                success = True
                break # 成功！立刻打破重试大循环
                
            except RuntimeError as e:
                # 捕获到了误差超过 0.032m 或 10度的异常
                self.get_logger().error(f"--> ❌ 误差超标，目标不稳定: {e}")
                if attempt < max_retries:
                    self.get_logger().info("--> 丢弃脏数据，准备重新触发视觉...")
                    time.sleep(0.5) # 给相机和环境光一点平息的时间
        ...
        if success:
            ...
            # 重试了设定的次数依然失败，果断终止，避免机械臂误动砸坏东西
            self.get_logger().fatal(f"🛑 精定位连续 {max_retries + 1} 次采样失败 (误差超标或超时)，已终止下探保护系统！")
        ...
        try:
            if frame_id == "base":
                ...
                coarse_target = TcpPose(
                    ...
                    y=float(flange_translation[1]) + 0.0,
