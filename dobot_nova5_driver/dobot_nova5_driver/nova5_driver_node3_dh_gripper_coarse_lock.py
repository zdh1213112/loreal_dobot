"""
功能说明：Nova5 粗定位到 D405 精定位抓取控制端（对应 D405 coarse_lock 基础版）。

主要流程：
1. 连接 Dobot Nova5 机械臂 TCP/IP 控制接口，并按配置初始化 DH 夹爪。
2. 订阅 `/target_pose_cam_coarse`，接收 D435 顶部相机粗定位结果，frame 为 `camera_d435_link`。
3. 使用 `handeye_base_to_d435` 把 D435 粗定位目标转换到机器人 base 坐标，生成粗定位靠近位姿。
4. 机械臂先执行粗定位移动，到目标上方/附近，为末端 D405 精定位创造视野。
5. 将锁定的粗定位目标通过 `/coarse_target_obj_for_d405` 发布给 D405，帮助 D405 限定目标。
6. 通过 `/trigger_d405_vision` 触发 D405 精定位节点采样。
7. 订阅 `/target_pose_cam_fine`，接收 D405 精定位抓取位姿，frame 为 `camera_d405_link`。
8. 使用 `handeye_flange_to_cam` 和当前法兰反馈，把 D405 精定位结果转换到 base 坐标并执行精抓。
9. 订阅 `/gripper_target_width`，根据视觉估计宽度控制 DH 夹爪开口/闭合。
10. 周期发布 `/nova5/current_tcp_pose`，用于外部观察当前 TCP 位姿。

匹配视觉端：`d405_local_ransac_coarse_lock.py`。
"""
...
class Nova5DriverNode(Node):
    ...
    def execute_latest_target_staged(self) -> None:
        ...
        self.get_logger().info(
            f"Executed staged fine planar move x={planar_target.x:.3f} y={planar_target.y:.3f} "
            f"z_hold={planar_target.z:.3f} rx={planar_target.rx:.2f} "
            f"ry={planar_target.ry:.2f} rz={planar_target.rz:.2f}"
        )
    ...
    def execute_coarse_then_fine_sequence(self) -> None: #粗定位--》精定位--》夹爪开合  执行流程
