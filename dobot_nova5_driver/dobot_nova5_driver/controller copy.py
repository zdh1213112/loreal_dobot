...
class DobotNova5Controller:
    ...
    def current_tcp_pose(self) -> TcpPose:
        ...
        return TcpPose(
            x=float(tcp[0]) / MM_PER_METER,
            y=float(tcp[1]) / MM_PER_METER,
            z=float(tcp[2]) / MM_PER_METER,
            rx=float(tcp[3]),
            ry=float(tcp[4]),
            rz=float(tcp[5]),
        )
    ...
    def read_pose(self) -> TcpPose:
        self._ensure_dashboard()
        with self._command_lock:
            values = self._raise_if_error(self.dashboard.GetPose(), "GetPose")
        if len(values) < 6:
            raise RuntimeError("GetPose did not return 6 pose values")
        return TcpPose(
    ...
    def move_linear_tcp(self, pose_m_deg: TcpPose, speed: int = 10, accel: Optional[int] = None) -> None:
        self._ensure_dashboard()
        with self._command_lock:
