...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("linear_acc", 60)
        self.declare_parameter("joint_speed", 60)
        self.declare_parameter("joint_acc", 60)
        ...
        self.declare_parameter("barcode_flip_max_steps", 4)
        self.declare_parameter("barcode_face_up_after_detect_deg", 90.0)
        self.declare_parameter(
            "default_transfer_joint",
            [220.0, 10.60, 57.08, 19.70, -90.0, -47.20], #主臂102中转检查点关节角度
        )
        self.declare_parameter(
            "default_place_pose",
        ...
        self.declare_parameter("secondary_startup_speed", 27)
        self.declare_parameter("secondary_joint_speed", 27)
        self.declare_parameter("secondary_joint_acc", 27)
    ...
    def _rotate_current_rx_with_log(self, delta_rx_deg: float, label: str) -> None:
        current_tcp = self.current_tcp_pose()
        target = TcpPose(
            current_tcp.x,
            current_tcp.y,
            current_tcp.z,
            normalize_angle_deg(current_tcp.rx + delta_rx_deg),
            current_tcp.ry,
            current_tcp.rz,
        )
        self._move_joint_pose_with_log(target, label)
    ...
    def _check_and_flip_until_barcode_face_up(
        ...
        for attempt in range(max_steps + 1):
            ...
            if self._wait_for_stable_barcode_detection(wait_timeout_s):
                self.get_logger().info(
                    f"Barcode face confirmed stable with {self._barcode_stable_hit_count()} detections."
                )
                if abs(face_up_after_detect_deg) > 1e-6:
                    self.get_logger().info(
                        f"Barcode detected; setting current TCP Rx += {face_up_after_detect_deg:.2f} deg to make barcode face upward."
                    )
                    self._rotate_current_rx_with_log(
                        face_up_after_detect_deg,
                        "Rotate detected barcode face upward by TCP Rx",
                    )
                return True
            if attempt >= max_steps:
                break
            self.get_logger().info(
                f"Barcode not stable; rotating {rotate_step_deg:.2f} deg for flip attempt {attempt + 1}/{max_steps}."
            )
    ...
    def execute_post_grasp_place_sequence(self, transfer_joint: list[float], place_pose: TcpPose) -> None:
        ...
        barcode_found = self._check_and_flip_until_barcode_face_up(
