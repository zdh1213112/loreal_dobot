"""
功能说明：Nova5 粗精定位抓取控制端（对应 D405 wait-lock + 拨散方向提示版）。
...
5. 订阅 `/target_singulation_hint`，接收 D405 根据目标 OBB/邻近点云估计的拨散方向。
6. 在真正抓取前，可根据 `pre_grasp_singulation_enabled` 执行预拨散动作：靠近目标、闭合夹爪轻推/拨开、抬起回退。
7. 拨散后重新触发 D405 精定位并采样，使用更新后的目标位姿执行精抓。
8. 抓取完成后执行用户配置的中转点、旋转检查和放置点流程。
9. 支持对拨散阶段单独配置速度、加速度、夹爪力、起点回退距离和推送距离。
10. 周期发布 `/nova5/current_tcp_pose`，用于外部观察当前 TCP 位姿。

匹配视觉端：`d405_local_ransac_coarse_lock_wait_lock_singulation.py`。
"""
...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("singulation_hint_topic", "/target_singulation_hint")
        ...
        self.declare_parameter("pre_grasp_singulation_enabled", True)
        self.declare_parameter("singulation_hover_above_target_m", 0.020)
        self.declare_parameter("singulation_contact_z_offset_m", 0.004)
        self.declare_parameter("singulation_retreat_height_m", 0.045)
        self.declare_parameter("singulation_nominal_body_width_m", 0.045)
        self.declare_parameter("singulation_edge_margin_m", 0.008)
        self.declare_parameter("singulation_push_margin_m", 0.012)
        self.declare_parameter("singulation_min_start_backoff_m", 0.020)
        self.declare_parameter("singulation_max_start_backoff_m", 0.045)
        self.declare_parameter("singulation_min_push_distance_m", 0.020)
        self.declare_parameter("singulation_max_push_distance_m", 0.040)
        self.declare_parameter("singulation_reacquire_delay_s", 0.20)
        self.declare_parameter("singulation_reacquire_samples", 2)
        self.declare_parameter("singulation_joint_speed", 12)
        self.declare_parameter("singulation_joint_acc", 12)
        self.declare_parameter("singulation_linear_speed", 8)
        self.declare_parameter("singulation_linear_acc", 10)
        self.declare_parameter("singulation_gripper_force", 20)
    ...
    def _estimate_target_body_width_m(self) -> float:
        nominal_width_m = float(self.get_parameter("singulation_nominal_body_width_m").value)
        tracked_width_m = self.latest_gripper_target_width_m()
        if tracked_width_m is None:
            return nominal_width_m
        estimated_body_width_m = tracked_width_m - 0.020
        return max(0.015, estimated_body_width_m)

    def _base_pull_direction_xy(self, target: TcpPose) -> np.ndarray:
        direction_xy = np.array([-target.x, -target.y], dtype=np.float64)
        norm = float(np.linalg.norm(direction_xy))
        if norm < 1e-6:
            return np.array([-1.0, 0.0], dtype=np.float64)
        return direction_xy / norm

    def _base_outward_direction_xy(self, target: TcpPose) -> np.ndarray:
        direction_xy = np.array([target.x, target.y], dtype=np.float64)
        norm = float(np.linalg.norm(direction_xy))
        if norm < 1e-6:
            return np.array([1.0, 0.0], dtype=np.float64)
    ...
    def _preferred_singulation_direction_xy(self, target: TcpPose) -> np.ndarray:
        outward_xy = self._base_outward_direction_xy(target)
        hinted_direction = self._latest_singulation_direction_xy()
        if hinted_direction is not None:
            hinted_xy = hinted_direction[:2]
            hinted_norm = float(np.linalg.norm(hinted_xy))
            if hinted_norm >= 1e-6:
                hinted_xy = hinted_xy / hinted_norm
                if np.dot(hinted_xy, outward_xy) < 0.0:
                    hinted_xy = -hinted_xy
                return hinted_xy
        return outward_xy
    ...
    def _singulation_start_backoff_m(self) -> float:
        body_width_m = self._estimate_target_body_width_m()
        edge_margin_m = float(self.get_parameter("singulation_edge_margin_m").value)
        min_backoff_m = float(self.get_parameter("singulation_min_start_backoff_m").value)
        max_backoff_m = float(self.get_parameter("singulation_max_start_backoff_m").value)
        planned_backoff_m = 0.5 * body_width_m + edge_margin_m
        return max(min_backoff_m, min(max_backoff_m, planned_backoff_m))

    def _singulation_push_distance_m(self) -> float:
        body_width_m = self._estimate_target_body_width_m()
        push_margin_m = float(self.get_parameter("singulation_push_margin_m").value)
        min_push_m = float(self.get_parameter("singulation_min_push_distance_m").value)
        max_push_m = float(self.get_parameter("singulation_max_push_distance_m").value)
        planned_push_m = 0.5 * body_width_m + push_margin_m
        return max(min_push_m, min(max_push_m, planned_push_m))
    ...
    def _execute_pre_grasp_singulation(self, target: TcpPose) -> TcpPose:
        ...
        pull_dir_xy = self._preferred_singulation_direction_xy(target)
        start_backoff_m = self._singulation_start_backoff_m()
        push_distance_m = self._singulation_push_distance_m()

        start_x = target.x - pull_dir_xy[0] * start_backoff_m
        start_y = target.y - pull_dir_xy[1] * start_backoff_m
        end_x = target.x + pull_dir_xy[0] * push_distance_m
        end_y = target.y + pull_dir_xy[1] * push_distance_m
        ...
        self.get_logger().info(
            ...
            f"body_width={self._estimate_target_body_width_m():.3f}m "
            f"start_backoff={start_backoff_m:.3f}m "
            f"push_distance={push_distance_m:.3f}m "
            f"pull_dir=({pull_dir_xy[0]:.3f}, {pull_dir_xy[1]:.3f})"
        )
