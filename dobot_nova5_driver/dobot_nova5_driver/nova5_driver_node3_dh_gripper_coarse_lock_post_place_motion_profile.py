"""
功能说明：Nova5 粗定位到 D405 精定位抓取控制端（对应 D405 wait-lock 增强版）。
...
4. 机械臂执行粗定位移动后，发布 `/coarse_target_obj_for_d405`，让 D405 wait-lock 版本等待并使用最新锁定目标。
5. 通过 `/trigger_d405_vision` 触发 D405 精定位，并支持多次精定位采样/稳定性检查/失败重试。
6. 订阅 `/target_pose_cam_fine`，接收 D405 精定位抓取位姿并转换到 base 坐标执行精抓。
7. 订阅 `/gripper_target_width`，用视觉估计宽度控制 DH 夹爪。
8. 抓取完成后执行预设中转点、末端旋转、放置点等 post-place 搬运流程。
9. 配置独立直线/关节速度和加速度 motion profile，便于粗定位、精抓、搬运阶段调速。
10. 周期发布 `/nova5/current_tcp_pose`，用于外部观察当前 TCP 位姿。

匹配视觉端：`d405_local_ransac_coarse_lock_wait_lock.py`。
"""
...
class Nova5DriverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("fine_tool_to_flange_z_m", 0.227) # 末端D405抓取时，夹爪尖端到法兰中心Z差值
