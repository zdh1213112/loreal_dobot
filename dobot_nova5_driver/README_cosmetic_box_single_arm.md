...
6. 到达中转关节后、开始自适应 X+ 靠近前即打开扫码窗口。如果靠近过程中已经读到稳定条码，靠近动作正常完成后直接进入扫码后 XYZ 流程。尚未读到时沿 J6 负方向点动寻找下一面，并在约 20 ms 的监控周期内检查条码。旋转途中一旦识别成功就立即停止 J6，然后对齐到最近的标准 90° 面位。没有扫码时不执行额外回正动作；下一轮根据最初中转 J6 的整数 90° 锚点计算剩余转角，自动扣除上一轮点动超调。只有真正扫码成功后才执行标准面对齐，随后进入 XYZ 与 User Ry `-90°` 翻转，因此节省无效动作且最终条码面正正朝上。
7. 保持扫码后的姿态角，先验证完整目标的逆解，再按 User 0 / Tool 1 点到点移动到 `(503, 121, 471) mm`。不再逐轴 MoveJog，避免某个中间 XYZ 状态导致控制器返回 `-1`。
8. 到达 `(503,121,471) mm` 后恢复旧程序的旋转方式：执行 `MoveJog("Ry-", coordtype=1, user=0, tool=1)`，保持 Tool 1 的 TCP XYZ 不主动改变，沿用户坐标系 Ry 负方向旋转约 `-90°` 后停止。旋转使用独立的 `face_up_rotation_speed_factor`（默认 35%），超时为 60 秒；该动作不再生成分段笛卡尔姿态，也不做中间姿态 IK，结束后检查旋转角度及 TCP XYZ 漂移。
...
- `scanner_approach_speed_factor=35`：只控制中转点后沿 User X+ 靠近扫码器的速度。
- `barcode_j6_speed_factor=35`：只控制 J6 多面找码，以及扫码成功后的标准面对齐速度；设置过高会增大急停超调。
- `face_up_rotation_speed_factor=35`：只控制扫码后保持 TCP XYZ 不动的 User Ry 末端翻转速度。
...
界面中可以修改初始关节、中转关节、扫码后用户 XYZ、最终放置位姿、普通运动速度、扫码器靠近速度、J6 找码速度、扫码后 Ry 翻转速度、夹爪力、抓取 Z 修正、TCP 最低安全 Z、抬升高度及扫码稳定参数。下降到位后，程序会在原位等待夹爪完成闭合并检查 `state=2`，确认夹持后才允许抬升；抬升后会再次检查是否掉落。建议先“只采样视觉”，确认结果后再执行完整一轮。
