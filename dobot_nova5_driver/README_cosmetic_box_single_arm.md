...
5. 视觉同时发布点云包围盒长边。抓取并抬起后移动到中转关节角 `(14, -29, -99, 39, 88, 10)`，再沿 User 0 的 X+ 自动靠近扫码器。位移使用 `中转中心距离 - 盒长/2 - 扫码间隙`；默认即 `120 mm - 盒长/2 - 20 mm`。例如盒长 100 mm 时向 X+ 移动 50 mm，使盒子近侧面距扫码器约 20 mm。
...
- `scanner_face_clearance_m=0.020`：自适应靠近后盒子侧面与扫码器之间保留的间隙。
