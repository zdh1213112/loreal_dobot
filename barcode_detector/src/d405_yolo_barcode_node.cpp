...
public:
    BarcodeDetectorNode() : Node("barcode_detector_node"),
        ...
        try {
            ...
            RCLCPP_INFO(this->get_logger(), " D405 启动：720P + 防掉帧AE + 远距离微距");
        ...
        if (total_frames_ % 1 == 0) {    
            ...
            for (int i = 0; i < 8400; ++i) {
                ...
                if (max_score > 0.25) { 
                    ...
                    // 严密拦截：过滤掉宽度或高度铺满屏幕 80% 以上的离谱假框
                    if (w_real > 0 && h_real > 0 && w_real < image.cols * 0.8 && h_real < image.rows * 0.8) {
                        boxes.push_back(cv::Rect(left, top, w_real, h_real));
            ...
            for (int idx : indices) {
                ...
                // 严密防越界重构：确保 padding 后不会出现内存越界崩溃或 UI 异常
                int pad = 20;
                ...
                for (double angle : angles) {
                    ...
                    // 💡 升级 3：针对 848x480 分辨率下的远距离检测，强制将小目标放大 4 倍起步
