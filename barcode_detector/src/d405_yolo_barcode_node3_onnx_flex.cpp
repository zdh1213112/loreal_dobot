...
public:
    BarcodeDetectorNode()
        : Node("barcode_detector_node_onnx_flex"),
          env_(ORT_LOGGING_LEVEL_WARNING, "YOLO-ORT-FLEX") {

        publisher_ = this->create_publisher<std_msgs::msg::String>("detected_barcodes", 10);

        this->declare_parameter<int>("ort_threads", 1);
        this->declare_parameter<int>("timer_ms", 30);
        this->declare_parameter<int>("infer_period_ms", 100);
        this->declare_parameter<bool>("use_slicing", false);
        this->declare_parameter<bool>("show", true);
        this->declare_parameter<double>("display_scale", 0.5);

        ort_threads_ = std::max(1, static_cast<int>(this->get_parameter("ort_threads").as_int()));
        timer_ms_ = std::max(10, static_cast<int>(this->get_parameter("timer_ms").as_int()));
        infer_period_ms_ = std::max(30, static_cast<int>(this->get_parameter("infer_period_ms").as_int()));
        use_slicing_ = this->get_parameter("use_slicing").as_bool();
        show_ = this->get_parameter("show").as_bool();
        display_scale_ = std::min(1.0, std::max(0.1, this->get_parameter("display_scale").as_double()));
        ...
        try {
            session_options_.SetIntraOpNumThreads(ort_threads_);
            session_options_.SetInterOpNumThreads(1);
            session_options_.SetExecutionMode(ExecutionMode::ORT_SEQUENTIAL);
            session_options_.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
            session_ = std::make_unique<Ort::Session>(env_, model_path.c_str(), session_options_);
            initialize_model_io();
            RCLCPP_INFO(this->get_logger(), "ONNXRuntime 引擎加载成功: %s, threads=%d", model_path.c_str(), ort_threads_);
        ...
        timer_ = this->create_wall_timer(std::chrono::milliseconds(timer_ms_), std::bind(&BarcodeDetectorNode::process_frame, this));
        last_time_ = std::chrono::steady_clock::now();
        last_infer_time_ = last_time_ - std::chrono::milliseconds(infer_period_ms_);
    }
...
private:
    ...
    std::atomic_bool processing_frame_{false};
    ...
    void process_frame() {
        bool expected = false;
        if (!processing_frame_.compare_exchange_strong(expected, true)) {
            return;
        }

        struct FlagReset {
            std::atomic_bool& flag;
            ~FlagReset() { flag.store(false); }
        } reset{processing_frame_};
        ...
        const double since_last_infer_ms = std::chrono::duration<double, std::milli>(now - last_infer_time_).count();
        if (since_last_infer_ms >= static_cast<double>(infer_period_ms_)) {
            const auto t0 = std::chrono::steady_clock::now();
            last_dets_ = use_slicing_ ? run_sliced_infer(image) : run_yolo_infer(image);
            const auto t1 = std::chrono::steady_clock::now();
            last_infer_ms_ = std::chrono::duration<double, std::milli>(t1 - t0).count();
            last_infer_time_ = now;
        }

        for (const auto& d : last_dets_) {
            char score_text[32];
        ...
        if (!last_dets_.empty()) {
            ...
            char buf[128];
            snprintf(buf, sizeof(buf), "detected %zu barcodes", last_dets_.size());
            msg.data = buf;
        ...
        snprintf(hud_text, sizeof(hud_text), "FPS: %.1f | Infer: %.1fms | Dets: %zu | Input: %dx%d",
            current_fps_, last_infer_ms_, last_dets_.size(), input_width_, input_height_);
        ...
        if (show_) {
            ...
            cv::imshow("YOLO ONNX Flex Detection", display);
            cv::waitKey(1);
        }
    }
};
